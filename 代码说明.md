...
合作方名单(partners)
    ↓
创建 monitor_task(partner_ids, sources, max_pages, crawl_mode)
    ↓
list_first: source × industry_keyword_batch → raw(list)
    ↓ list_triage → investigation_queue → detail crawl（xhs：搜索页弹窗，非 goto explore）
legacy: partner × source → raw(legacy, 可选详情)
    ↓
NormalizeAdapter → PartnerMatcher（多方 match）→ candidates
    ↓
AnalyzePipeline（仅 detail / 非 noise 列表）→ intel_records
    ↓
...
| `monitor.*` | 默认源/页数；`crawl_mode`（list_first/legacy）；`priority_quota`；`task_timeout_sec` 为监测任务总上限；`analysis_timeout_sec` 为爬取完成后 AI 分析预留（默认 3600s）；爬取阶段会在总上限前预留分析时间并逐页检查超时 |
