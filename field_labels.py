...
FIELD_LABELS = {
    ...
    'search_url_template': _entry('搜索 URL 模板', 'crawl', 'text', '可选；占位符 {keyword}，列表通过搜索框+下拉加载'),
    ...
    'early_stop': _entry('分页见底早停', 'crawl', 'object', '含 enabled、分源策略字段'),
    'early_stop.enabled': _entry('启用早停', 'crawl', 'checkbox'),
    'early_stop.min_pages': _entry('最少采集页/轮', 'crawl', 'number'),
    'early_stop.empty_pages_threshold': _entry('连续空页阈值', 'crawl', 'number', '黑猫'),
    'early_stop.protect_first_page': _entry('保护第 1 页', 'crawl', 'checkbox', '防加载失败误停'),
    'early_stop.empty_page_retry': _entry('第 1 页空结果重试', 'crawl', 'number', '黑猫，默认 0 不重试'),
    'empty_search': _entry('空搜索行为', 'crawl', 'object', '无结果时是否误判登录'),
    'empty_search.login_on_missing_sid': _entry('无 sid 即等登录', 'crawl', 'checkbox', '默认 false：SUB 有效则跳过'),
    'early_stop.end_texts': _entry('见底文案列表', 'crawl', 'text', '小红书，如 - THE END -'),
    'early_stop.end_selectors': _entry('见底选择器', 'crawl', 'text', '小红书可选'),
    'early_stop.saturation_rounds': _entry('滚动饱和轮数', 'crawl', 'number', '小红书'),
    'investigation_detail': _entry('勘察弹窗详情', 'crawl', 'object', 'list_first investigation 弹窗路径'),
    'investigation_detail.dom_miss_skip': _entry('DOM 未找到跳过', 'crawl', 'checkbox'),
    'investigation_detail.dom_miss_research_threshold': _entry('重搜阈值', 'crawl', 'number', '同 keyword 累计 miss'),
    ...
    'batch_size': _entry('分析批大小', 'analysis', 'number'),
    'parallel_batches': _entry('并行批数', 'analysis', 'number', '同时发起的分析 API 请求数'),
    ...
    'confidence': _entry('置信度', 'intel'),
    'sentiment_label': _entry('情感', 'intel'),
    'sentiment_score': _entry('情感分数', 'intel'),
    'recency.enabled': _entry('启用时效降档', 'analysis', 'checkbox'),
    'recency.downgrade_days_high_to_medium': _entry('high→medium 天数', 'analysis', 'number'),
    'recency.downgrade_days_medium_to_low': _entry('medium→low 天数', 'analysis', 'number'),
    'recency.confidence_downgrade_threshold': _entry('低置信降档阈值', 'analysis', 'number'),
    'list_triage.batch_size': _entry('初筛批大小', 'analysis', 'number', '爬取阶段 list_triage 每批条数'),
    'list_triage.max_body_chars': _entry('初筛正文最大字符', 'analysis', 'number'),
    'list_triage.enabled': _entry('启用列表初筛', 'analysis', 'checkbox'),
    ...
    'crawl_duration_ms': _entry('爬取阶段耗时', 'monitor_run', 'number', '列表+初筛+勘察合计（毫秒）'),
    'analyze_duration_ms': _entry('情报分析耗时', 'monitor_run', 'number', '增量 AI 打标 wall time（毫秒）'),
    'error_message': _entry('Error Message', 'monitor_run', 'text', 'Failure summary and full stack trace when available'),
    ...
    'triage_high': _entry('初筛-高', 'monitor_run', 'number', '高优先级初筛计数'),
    'triage_medium': _entry('初筛-中', 'monitor_run', 'number', '中优先级初筛计数'),
    'triage_noise': _entry('初筛-噪声', 'monitor_run', 'number', '噪声初筛计数'),
    'needs_investigation_count': _entry('待勘察', 'monitor_run', 'number', '标记需深度勘察的条数'),
    ...
    'crawl_ms': _entry('列表爬取', 'monitor_run', 'number', '搜索列表页滚动抓取'),
    'list_crawl_ms': _entry('列表爬取', 'monitor_run', 'number', '搜索列表页滚动抓取'),
    'triage_ms': _entry('列表初筛', 'monitor_run', 'number', '爬取阶段 LLM 初筛'),
    'investigation_crawl_ms': _entry('详情勘察', 'monitor_run', 'number', '同页/弹窗详情抓取'),
    'intel_analyze_ms': _entry('情报分析', 'monitor_run', 'number', '增量 AI 情报打标'),
    'analyze_ms': _entry('情报分析', 'monitor_run', 'number', '增量 AI 情报打标（兼容字段）'),
...
ELEMENT_ID_TO_KEY = {
    ...
    'aiKey': 'api_key',
    'aiBatch': 'batch_size',
    'aiParallelBatches': 'parallel_batches',
    'aiBodyMax': 'max_body_chars',
    'aiTriageBatch': 'list_triage.batch_size',
    'aiTriageBodyMax': 'list_triage.max_body_chars',
    'aiTriageEnabled': 'list_triage.enabled',
